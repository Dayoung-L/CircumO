...
CircumPlot <-
  function(object,pchar=NULL,bg.points="red",ef=0.4,
           ...
           twodim=TRUE,bound=TRUE,labels=TRUE,reverse=FALSE){
    ...
    dev.new(title="  CIRCULAR POSITION  ",width=4,height=4)
    ...
    plot(c(-1.3,1.3),c(-1.3,1.3),type="n",bty="n",axes=FALSE);
    ...
    segments(c(rep(0,dim(object$R)[1])),c(rep(0,dim(object$R)[1])),angular.points[,1]*com.ind,angular.points[,2]*com.ind,col=color,lty="dashed")
    if(labels==TRUE){
